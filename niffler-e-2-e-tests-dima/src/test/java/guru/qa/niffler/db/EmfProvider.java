...
public enum EmfProvider {
    ...
    private final Map<JdbcUrl, EntityManagerFactory> store = new ConcurrentHashMap<>();

    public EntityManagerFactory emf(JdbcUrl jdbcUrl) {
        return store.computeIfAbsent(jdbcUrl, k -> {
            ...
            return new ThreadSafeEntityManagerFactory(Persistence.createEntityManagerFactory("niffler-st4", settings));
        });
    }
